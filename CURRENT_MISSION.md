...
### Restart Incident Visibility v1
Current objective: make unexpected systemd process failure durable and monitor-visible before authorizing an overnight gate. A post-stop evidence adapter appends `PROCESS_INCIDENT` only when systemd reports a non-successful service result. Operational Monitoring classifies an incident in the current attempt as `CRITICAL / FAILED / PROCESS_FAILURE` and retains the immediately previous failed attempt as `PREVIOUS_PROCESS_FAILURE` warning throughout the restarted attempt, including after a later `MAX_BARS` completion.

The recorder is evidence-only: it cannot classify alerts, restart trading, mutate continuity, generate signals, alter ordering, change risk policy or submit orders. Its own failure is ignored by the service manager so observability cannot change the existing `Restart=on-failure` lifecycle. Local TDD is GREEN with 39/39 focused tests, 605/605 complete Python 3.12.13 tests and native systemd 255 syntax verification in the isolated validation environment.

Pending before overnight authorization: commit/push, Windows/Python 3.14.6 validation, exact-commit cloud deployment, native target-unit verification and a controlled failure/restart evidence probe proving `PROCESS_INCIDENT -> CRITICAL -> PREVIOUS_PROCESS_FAILURE WARNING` while `REAL_orders=0` remains invariant.
