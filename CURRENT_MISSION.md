...
### Overnight Soak Failure Closure v1 — Local Implementation
Current objective: close only the defects proven by the overnight evidence before another cloud endurance attempt. The two-second reorder policy remains unchanged. A truly late Coinbase trade now raises a typed fail-closed error carrying trade time, trade/active buckets, latest-seen time, event-time watermark, configured reorder window and measured lateness; the forward audit records that evidence as `LATE_TRADE_REJECTED`, closes with `ORDERING_FATAL`, preserves continuity and still exits non-zero for supervision.

A controlled SIGINT now closes the latest newly opened session as `OPERATOR_STOP` using already-durable audit/checkpoint evidence before returning status 130. Operational Monitoring classifies it as closed `STOPPED`/`WARNING`, never as stale `RUNNING`; fatal ordering remains `FAILED`/`CRITICAL`. The deterministic report separates structural completeness from gate acceptance: only a complete, zero-real-order `MAX_BARS` session may be `PASS`; `OPERATOR_STOP` and `ORDERING_FATAL` are complete but non-passing.

The supervision envelope now uses `StartLimitIntervalSec=infinity` and `StartLimitBurst=2`: one reviewed initial start plus at most one automatic recovery start until an operator explicitly resets the budget while inactive. This preserves one recovery opportunity but prevents the seven-restart/unbounded-duration failure mode. Strategy, Feed Health, hybrid recovery, Risk Engine, PaperBroker and the structural `REAL_orders=0` boundary remain unchanged.

Local TDD and regression validation pass 97/97 focused tests and the complete 615/615 Python 3.12.13 suite. The exact diff also applies cleanly to a detached overnight-base worktree, where the same 97/97 and 615/615 results reproduce with clean whitespace and installer shell syntax. Pending boundaries are exact patch application plus Windows validation, reviewed commit/push, non-activating cloud deployment validation and a short diagnostic PAPER probe. Another overnight run is not authorized until those gates are clean.
