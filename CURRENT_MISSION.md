...
### Bounded Multi-Hour Cloud PAPER Soak v1
Current objective: collect 180 fresh BTC-USD one-minute bars, approximately three hours, through the proven non-root systemd service while the independent monitor runs every minute. This is an operational endurance and transport-quality gate, not a profitability or strategy-tuning experiment.

The session bound now comes from one committed, root-owned deployment configuration. Cloud Runtime Readiness and the forward runner consume the same value, eliminating the prior duplicated ten-bar literals while preserving PAPER mode, disabled real execution, persistent continuity, failure-only restart and boot-disabled activation. Acceptance requires 180/180 processed, complete audit, `MAX_BARS`, `observed_gap=0.0m`, zero recovery failures, `REAL_orders=0`, no `CRITICAL` monitoring state and explicit transport-quality reporting. Overnight, 24-hour and multi-day gates remain unauthorized until this shorter gate passes.

Preparation passes 25/25 focused supervision/readiness tests and the complete 597/597 suite both in the isolated Python 3.12.13 environment and on Windows/Python 3.14.6. Exact-commit cloud validation, native systemd verification and the live 180-bar evidence gate remain pending.
