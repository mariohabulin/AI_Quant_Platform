...
### AI-Driven v2 Risk and Execution — WINDOWS INTEGRATED
...
- [x] Implement a synthetic-only adapter with adverse taker fees, spread and
  ...
  exits.
- [x] Reproduce the risk/execution focused and complete Windows regression,
  inspect its hash-bound nonexecuting declaration, then commit and push the
  exact milestone `f8d2436` after 1,348/1,348 tests passed without opening the
  locked dataset.

### AI-Driven v2 Development/Evaluation Partition — WINDOWS INTEGRATED

- [x] Freeze protocol `kraken-btc-eth-xrp-ai-driven-v2-partition-v1` around
  calendar-only development, inspected calibration and genuinely untouched
  evaluation identities before reading strategy performance.
- [x] Place the inspected BTC v1 evidence SHA-256
  `56710a21a423a63963e5c97ab6ca956021f9cd7a7d494c3f29a197068367ff60`
  wholly in calibration and permanently prohibit an unseen label for it.
- [x] Reserve `2025-04-01T00:00:00Z` through `2026-04-01T00:00:00Z`
  exclusive as sealed one-time evaluation; preserve exact gaps and reset all
  feature, state, position and risk context at every gap and partition.
- [x] Implement a synthetic-only timestamp contract and hash-bound review with
  no external dataset, partition materialization, performance or runner.
- [x] Reproduce the partition focused and complete Windows regression, inspect
...
- [x] Verify complete locked files by opaque SHA-256 while parsing only the
  development prefix before `2024-04-01T00:00:00Z`; prohibit calibration and
  evaluation OHLCV conversion.
- [x] Freeze shared USD 5,000 research cash/risk, exits-before-entries,
  BTC/ETH/XRP same-time order, gap halt, unresolved terminal-position and
  adverse marked-equity semantics without a parameter sweep or promotion.
...
- [x] Decide to continue with bounded hybrid discovery: one shared strategy-
  family catalog and hard portfolio safety envelope with asset/regime-specific
  BTC/ETH/XRP routing and first-class `HOLD_CASH`.

### AI-Driven v2 Hybrid Strategy Discovery and Learning — LOCAL SYNTHETIC REVIEW

- [x] Freeze protocol
  `kraken-btc-eth-xrp-ai-driven-v2-hybrid-strategy-discovery-learning-v1`
  without registering a hypothesis or opening development data.
- [x] Bound the catalog to capitulation recovery, trend-pullback continuation,
  range mean reversion and volatility breakout with family-owned causal regimes
  and permitted indicator primitives.
- [x] Limit one round to six hypotheses, two variants per family and four routes
  per asset; limit protocol v1 to two separately authorized rounds and twelve
  cumulative hypotheses.
- [x] Preserve the shared `0.50%` position-risk, `1.50%` portfolio-risk,
  three-position, one-third-notional and adverse-cost safety envelope while
  requiring new family-specific causal signal and execution contracts.
- [x] Implement canonical hypothesis-manifest validation, Reference-A identity
  reuse rejection, immutable offline-feedback lineage and hash-bound
  nonexecuting review.
- [x] Reproduce focused/full regression and hash-bound review on Windows, then
  commit and push exact hybrid protocol milestone `20d6767`.
- [x] Pre-register bounded hybrid discovery Round 1 with four exact hypotheses,
  causal regime/signal/execution specifications, costs, chronological slices
  and numeric development-interest gates before any data access.
- [x] Freeze protocol
  `kraken-btc-eth-xrp-ai-driven-v2-hybrid-discovery-round-1-v1`, canonical
  parent-manifest SHA-256 and full configuration SHA-256 with Reference A as
  immutable feedback lineage only.
- [x] Require route-level absolute gates and a separate portfolio review when
  multiple families pass for one asset; retain no return leaderboard or
  automatic strategy selection.
- [x] Reproduce Round 1 focused/full regression and hash-bound review on
  Windows, then commit and push exact pre-registration milestone `b6ea2ab`.
- [x] Implement the four exact causal feature, regime and entry-signal paths
  using synthetic formula, state and prefix-causality tests only; keep every
  real Development value closed.
- [x] Reproduce Round 1 Causal Signals focused/full regression and hash-bound
  review on Windows, then commit and push exact signal-only milestone `05e5f45`.
- [x] Implement the four family-specific next-open execution adapters with
  synthetic gap, stop, target, cost and shared-risk tests only.
- [x] Reproduce Round 1 Family Execution focused/full regression and hash-bound
  review on Windows, then commit and push the exact execution-only milestone.
- [x] Build a one-shot development-only discovery runner only after Round 1
  components pass prefix-causality, safety and hash-binding review.
- [x] Reproduce Round 1 Discovery Runner focused/full regression and hash-bound
  review on Windows, then commit/push before any Development authorization.
- [x] Confirm clean repository plus absent final/staging evidence, then make a
  separate operator decision whether to authorize exactly one Round 1
  Development discovery run.
- [x] Execute Round 1 once on Development, lock canonical report SHA-256
  `3ce14fda95f657c0b671b74c702d55ec4102da303e9e033ebaf0e02ff5c2fa9b`
  and preserve zero Calibration/Evaluation access and zero real orders.
- [x] Attribute all 12 route failures without ranking: two single-gate breakout
  observations, three no-trade range routes and two XRP negative-expectancy
...
- [x] Reproduce Round 2 pre-registration focused/full regression and hash-bound
  review on Windows, then commit/push exact registration milestone `903a447`.
- [x] Implement Round 2 Causal Signals for the three exact registered families
  using synthetic formula, state-transition and prefix-causality tests only.
- [x] Reproduce Round 2 Causal Signals focused/full regression and hash-bound
  review on Windows, then commit/push exact signal milestone `9f58a8b`.
- [x] Implement Round 2 Family Execution for three exact registered families
  with synthetic cost, stop, target, sizing and protective-path tests only.
- [x] Reproduce Round 2 Family Execution focused/full regression and hash-bound
  review on Windows, then commit/push exact execution milestone `5c226f1`.
- [x] Locally implement a one-shot Development-only Round 2 Discovery Runner
