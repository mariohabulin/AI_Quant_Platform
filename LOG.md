...
- Changed systemd start limiting to `StartLimitIntervalSec=infinity` and `StartLimitBurst=2`, allowing the reviewed initial start plus one automatic recovery start. The runbook requires explicit `reset-failed` before a new inactive activation and prohibits resetting the budget during a gate.
