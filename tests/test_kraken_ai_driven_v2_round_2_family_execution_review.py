...
COMPONENT = ROOT / "src" / "kraken_ai_driven_v2_round_2_family_execution.py"
...
def test_changed_round_2_causal_parent_binding_is_rejected(tmp_path, binding_index):
...
def test_bad_causal_parent_review_is_rejected():
    with pytest.raises(RuntimeError, match="parent review mismatch"):
        review_declaration(parent_reviewer=lambda: {"status": "WRONG"})
...
def test_protocol_and_project_documents_record_execution_only_boundary():
    ...
    assert "three exact family execution adapters" in protocol
    assert "Open priority is stop gap" in protocol
    assert "same-bar stop/target conflict is `STOP_FIRST`" in protocol
    assert "three Round 2 execution components implemented: `true`" in protocol
    ...
    for name in (
        ...
        assert "Round 2 Family Execution" in text
