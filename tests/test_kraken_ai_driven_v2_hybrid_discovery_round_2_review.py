import json
import os
import sys
from pathlib import Path

import pytest


sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "src")))

from kraken_ai_driven_v2_hybrid_discovery_round_2 import (
    PROTOCOL_ID,
    ROUND_ID,
    STATUS,
)
from kraken_ai_driven_v2_hybrid_discovery_round_2_review import (
    COMPONENT_BINDINGS,
    ROUND_2_COMPONENT_NORMALIZED_SHA256,
    ROUND_2_PROTOCOL_NORMALIZED_SHA256,
    load_round_2_protocol,
    main,
    normalized_text_sha256,
    review_declaration,
)


ROOT = Path(__file__).resolve().parents[1]
PROTOCOL = (
    ROOT
    / "KRAKEN_BTC_ETH_XRP_AI_DRIVEN_V2_HYBRID_DISCOVERY_ROUND_2_PROTOCOL_V1.md"
)
COMPONENT = ROOT / "src" / "kraken_ai_driven_v2_hybrid_discovery_round_2.py"


def test_review_hash_binds_budget_closure_round_1_and_round_2_artifacts():
    _, protocol_digest = load_round_2_protocol(PROTOCOL)

    assert protocol_digest == ROUND_2_PROTOCOL_NORMALIZED_SHA256
    assert normalized_text_sha256(COMPONENT) == (
        ROUND_2_COMPONENT_NORMALIZED_SHA256
    )
    assert len(COMPONENT_BINDINGS) == 8
    for binding in COMPONENT_BINDINGS:
        assert normalized_text_sha256(ROOT / binding["path"]) == binding["sha256"]


@pytest.mark.parametrize("binding_index", range(8))
def test_changed_parent_binding_is_rejected(tmp_path, binding_index):
    changed_paths = [ROOT / item["path"] for item in COMPONENT_BINDINGS]
    source = changed_paths[binding_index]
    changed = tmp_path / source.name
    changed.write_text(
        source.read_text(encoding="utf-8") + "changed\n", encoding="utf-8"
    )
    changed_paths[binding_index] = changed

    with pytest.raises(RuntimeError, match="SHA256 mismatch"):
        review_declaration(
            *changed_paths,
            round_2_protocol_path=PROTOCOL,
            round_2_component_path=COMPONENT,
        )


@pytest.mark.parametrize(
    "source,keyword",
    [
        (PROTOCOL, "Round 2 protocol SHA256"),
        (COMPONENT, "Round 2 component SHA256"),
    ],
)
def test_changed_round_2_artifact_is_rejected(tmp_path, source, keyword):
    changed = tmp_path / source.name
    changed.write_text(
        source.read_text(encoding="utf-8") + "changed\n", encoding="utf-8"
    )
    kwargs = {
        "round_2_protocol_path": PROTOCOL,
        "round_2_component_path": COMPONENT,
    }
    if source == PROTOCOL:
        kwargs["round_2_protocol_path"] = changed
    else:
        kwargs["round_2_component_path"] = changed

    with pytest.raises(RuntimeError, match=keyword):
        review_declaration(**kwargs)


def test_changed_dependency_review_is_rejected():
    def changed_review():
        return {
            "status": "CHANGED",
            "closure_component_sha256_match": True,
            "closure_document_sha256_match": True,
        }

    with pytest.raises(RuntimeError, match="closure review status"):
        review_declaration(parent_reviewer=changed_review)


def test_review_confirms_bounded_registration_without_component_or_run_authority():
    declaration = review_declaration()

    assert declaration["status"] == (
        "KRAKEN_AI_V2_HYBRID_DISCOVERY_ROUND_2_REVIEWED_"
        "COMPONENT_IMPLEMENTATION_REQUIRED"
    )
    assert declaration["round_2_status"] == STATUS
    assert declaration["protocol_id"] == PROTOCOL_ID
    assert declaration["round_id"] == ROUND_ID
    assert declaration["hypothesis_count"] == 3
    assert declaration["asset_route_counts"] == {
        "BTC-USD": 3,
        "ETH-USD": 3,
        "XRP-USD": 1,
    }
    assert declaration["cumulative_hypothesis_count"] == 7
    assert declaration["rounds_registered_under_protocol"] == 2
    assert declaration["parent_source_binding_matches"]
    assert all(declaration["parent_source_binding_matches"].values())
    assert declaration["round_2_protocol_sha256_match"] is True
    assert declaration["round_2_component_sha256_match"] is True
    assert declaration["round_1_closure_review_passed"] is True
    assert declaration["remaining_discovery_budget_verified"] is True
    assert declaration["route_dispositions_frozen"] is True
    assert declaration["round_1_gates_weakened"] is False
    assert declaration["cost_profiles_changed"] is False
    assert declaration["development_slices_changed"] is False
    assert declaration["round_2_manifest_registered"] is True
    assert declaration["regime_components_implemented"] is False
    assert declaration["signal_components_implemented"] is False
    assert declaration["execution_components_implemented"] is False
    assert declaration["discovery_runner_implemented"] is False
    assert declaration["dataset_opened"] is False
    assert declaration["development_data_opened"] is False
    assert declaration["calibration_data_opened"] is False
    assert declaration["evaluation_data_opened"] is False
    assert declaration["development_run_authorized"] is False
    assert declaration["performance_evaluation_executed"] is False
    assert declaration["automatic_ranking_authorized"] is False
    assert declaration["candidate_v2_authorized"] is False
    assert declaration["live_execution_authorized"] is False
    assert declaration["next_stage"] == (
        "IMPLEMENT_ROUND_2_CAUSAL_COMPONENTS_SYNTHETIC_ONLY"
    )


def test_review_cli_prints_only_declaration(capsys):
    result = main([])
    payload = json.loads(capsys.readouterr().out)

    assert payload == result
    assert payload["round_2_manifest_registered"] is True
    assert payload["development_data_opened"] is False
    assert payload["evaluation_data_opened"] is False


def test_protocol_and_project_documents_record_exact_round_2_boundary():
    protocol = PROTOCOL.read_text(encoding="utf-8")
    assert PROTOCOL_ID in protocol
    assert ROUND_ID in protocol
    assert "three new hypotheses" in protocol
    assert "seven of the maximum twelve" in protocol
    assert "Round 1 rerun authorization" in protocol
    assert "Development data opened: `false`" in protocol
    assert "Evaluation data opened: `false`" in protocol
    assert "Candidate v2" in protocol

    for name in (
        "VISION.md",
        "ARCHITECTURE.md",
        "ROADMAP.md",
        "CURRENT_MISSION.md",
        "LOG.md",
    ):
        text = (ROOT / name).read_text(encoding="utf-8")
        assert PROTOCOL_ID in text
        assert "Round 2" in text
        assert "three" in text.lower()
        assert "3ce14fda95f657c0b671b74c702d55ec4102da303e9e033ebaf0e02ff5c2fa9b" in text
        assert "Candidate v2" in text
