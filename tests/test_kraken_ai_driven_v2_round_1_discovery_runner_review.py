...
PROTOCOL = ROOT / "KRAKEN_BTC_ETH_XRP_AI_DRIVEN_V2_ROUND_1_DISCOVERY_RUNNER_PROTOCOL_V1.md"
COMPONENT = ROOT / "src" / "kraken_ai_driven_v2_round_1_discovery_runner.py"
...
def test_review_confirms_runner_but_keeps_data_and_authorization_closed():
    ...
    assert declaration["route_count"] == 12
...
def test_parent_review_cannot_claim_data_or_execution():
    def bad_parent():
        return {
            "status": "KRAKEN_AI_V2_ROUND_1_FAMILY_EXECUTION_REVIEWED_DISCOVERY_RUNNER_REQUIRED",
...
def test_protocol_and_project_documents_record_unexecuted_runner_boundary():
    ...
    assert "twelve asset-family route reports" in protocol
    ...
    for name in ("VISION.md", "ARCHITECTURE.md", "ROADMAP.md", "CURRENT_MISSION.md", "LOG.md"):
        ...
        assert "Round 1 Discovery Runner" in text
        assert "12" in text
