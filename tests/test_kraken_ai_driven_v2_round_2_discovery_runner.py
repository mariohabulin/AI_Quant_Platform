...
from kraken_ai_driven_v2_hybrid_discovery_round_2 import ROUND_2_HYPOTHESES
...
from kraken_ai_driven_v2_round_2_discovery_runner import (
    ...
    KrakenAIDrivenV2Round2DiscoveryRunner,
...
def test_configuration_freezes_seven_routes_and_no_execution_authorization():
...
class _CapitulationSignalEngine:
    def generate(self, family, frame):
        ...
        hypothesis = next(item for item in ROUND_2_HYPOTHESES if item["family_id"] == family)
        for column, value in (
        ...
        result.loc[first, SETUP_TIMESTAMP_COLUMN] = first - pd.Timedelta(days=2)
        return result
...
def test_profile_executes_following_open_and_entry_bar_target_without_force_close():
    ...
    runner = KrakenAIDrivenV2Round2DiscoveryRunner(signal_engine_factory=_CapitulationSignalEngine)
...
def test_wrong_authorization_fails_before_dataset_or_evidence_access(tmp_path):
    reader = _FailIfRead()
    runner = KrakenAIDrivenV2Round2DiscoveryRunner(dataset_reader=reader)

    with pytest.raises(PermissionError, match="Exact Round 2 discovery"):
...
class _EvidenceRunner(KrakenAIDrivenV2Round2DiscoveryRunner):
...
def test_exact_authorization_records_canonical_one_shot_evidence(tmp_path):
    ...
    locked = KrakenAIDrivenV2Round2DiscoveryEvidenceLock().lock(final)
