...
COMPONENT = ROOT / "src" / "kraken_ai_driven_v2_round_1_causal_signals.py"


def test_review_hash_binds_round_1_registration_and_signal_artifacts():
...
def test_changed_round_1_parent_binding_is_rejected(tmp_path, binding_index):
...
def test_review_confirms_signals_but_keeps_execution_and_data_closed():
    ...
    assert declaration["parent_round_1_review_passed"] is True
...
def test_protocol_and_project_documents_record_signal_only_boundary():
    ...
    assert "four deterministic signal paths" in protocol
    assert "immediate next completed bar" in protocol
    assert "rolling baseline current bar included: `false`" in protocol
    assert "execution components implemented: `false`" in protocol
    ...
    for name in (
        ...
        assert "Round 1 Causal Signals" in text
