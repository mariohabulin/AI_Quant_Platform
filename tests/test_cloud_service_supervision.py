...
def test_paper_service_uses_reviewed_three_day_soak_bound():
    environment = _environment("ai-alpha-paper.env")

    assert environment == {"AI_ALPHA_SESSION_BARS": "4320"}
    assert int(environment["AI_ALPHA_SESSION_BARS"]) == 3 * 24 * 60
...
def test_runbook_requires_reviewed_three_day_gate_evidence():
    runbook = _unit("README.md")

    assert "AI_ALPHA_SESSION_BARS=4320" in runbook
    assert "4,320/4,320 processed bars" in runbook
    assert "successful 1,440-bar gate" in runbook
    assert "three-day" in runbook
    ...
    assert "CPU time and memory peak" in runbook
    assert "not strategy profitability evidence" in runbook
    assert "unattended production" in runbook
