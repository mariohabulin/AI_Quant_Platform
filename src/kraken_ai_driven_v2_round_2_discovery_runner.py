"""One-shot Development discovery runner for Kraken AI-driven V2 Round 2."""
...
try:
    ...
    from kraken_ai_driven_v2_hybrid_discovery_round_2 import (
        ...
        PROTOCOL_ID as ROUND_2_PROTOCOL_ID,
        ROUND_ID,
    )
...
DISCOVERY_RUNNER_PROTOCOL_ID = (
    "kraken-btc-eth-xrp-ai-driven-v2-round-2-discovery-runner-v1"
)
DISCOVERY_RUN_ID = "kraken-ai-v2-round-2-development-discovery-v1"
AUTHORIZATION_PHRASE = "EXECUTE_KRAKEN_AI_V2_ROUND_2_DISCOVERY_ONCE"
INITIAL_CAPITAL = 5000.0
QUOTE_CURRENCY = "USD_RESEARCH_NOTIONAL"
EVIDENCE_DIRECTORY_NAME = "round_2_development_discovery_v1"
STAGING_DIRECTORY_NAME = ".round_2_development_discovery_v1.staging"
REPORT_FILENAME = "kraken_ai_v2_round_2_discovery_report.json"
REPORT_SHA256_FILENAME = "kraken_ai_v2_round_2_discovery_report.sha256"
COST_PROFILE_ORDER = (BASELINE_COST_PROFILE_ID, STRESS_COST_PROFILE_ID)
REGISTERED_FAMILIES_BY_ASSET = {
    asset: tuple(
        hypothesis["family_id"]
        for hypothesis in ROUND_2_HYPOTHESES
        if asset in hypothesis["asset_scope"]
    )
...
ROUTE_ORDER = tuple(
    f"{asset}|{family}"
    for asset in ASSET_ORDER
    for family in REGISTERED_FAMILIES_BY_ASSET[asset]
)
...
def discovery_runner_configuration():
    return {
        ...
        "round_2_protocol_id": ROUND_2_PROTOCOL_ID,
        "round_id": ROUND_ID,
        ...
        "registered_families_by_asset": {
            asset: list(REGISTERED_FAMILIES_BY_ASSET[asset]) for asset in ASSET_ORDER
        },
        ...
        "route_interest_gates": dict(ROUND_2_ROUTE_INTEREST_GATES),
        "round_interest_gates": dict(ROUND_2_SELECTION_GATES),
...
class RecordedRound2DiscoveryEvidence:
    report_path: Path
    checksum_path: Path
    report_sha256: str
    round_status: str
    ...
    status: str = "KRAKEN_AI_V2_ROUND_2_DISCOVERY_EVIDENCE_RECORDED"
...
@dataclass(frozen=True)
class LockedRound2DiscoveryEvidence:
    report_path: Path
    checksum_path: Path
    report_sha256: str
    report: dict
    status: str = "KRAKEN_AI_V2_ROUND_2_DISCOVERY_EVIDENCE_LOCK_PASS"
...
def evaluate_route_interest(baseline, stress):
    ...
    gates = ROUND_2_ROUTE_INTEREST_GATES
    checks = {
...
def summarize_round_interest(route_results):
    ...
    round_pass = (
        len(eligible_routes) >= ROUND_2_SELECTION_GATES["minimum_eligible_route_count"]
        and len(eligible_assets) >= ROUND_2_SELECTION_GATES["minimum_eligible_asset_count"]
    )
    ...
    if not round_pass:
        ...
        status = "KRAKEN_AI_V2_ROUND_2_DEVELOPMENT_INTEREST_PORTFOLIO_REVIEW_REQUIRED"
        next_action = "SEPARATE_PORTFOLIO_REVIEW_REQUIRED"
    else:
        status = "KRAKEN_AI_V2_ROUND_2_DEVELOPMENT_INTEREST_RETAINED"
...
class KrakenAIDrivenV2Round2DiscoveryEvidenceLock:
    """Independently lock canonical one-shot Round 2 discovery evidence."""
    ...
    def lock(self, evidence_directory):
        ...
        try:
            ...
            raise ValueError("Unable to read Round 2 discovery evidence.") from exc
        digest = hashlib.sha256(report_bytes).hexdigest()
        if checksum != [digest, REPORT_FILENAME]:
            raise ValueError("Round 2 discovery evidence SHA-256 sidecar mismatch.")
        try:
            report = json.loads(report_bytes.decode("utf-8"))
        except (UnicodeError, json.JSONDecodeError) as exc:
            raise ValueError("Unable to decode Round 2 discovery evidence.") from exc
        if report_bytes != canonical_json_bytes(report):
            raise ValueError("Round 2 discovery report bytes are not canonical.")
        expected = {
            "schema_version": SCHEMA_VERSION,
            "discovery_runner_protocol_id": DISCOVERY_RUNNER_PROTOCOL_ID,
            "discovery_run_id": DISCOVERY_RUN_ID,
        ...
        for key, value in expected.items():
            if report.get(key) != value:
                raise ValueError(f"Round 2 discovery evidence field is invalid: {key}.")
        if len(report.get("route_results", ())) != len(ROUTE_ORDER):
            raise ValueError("Round 2 discovery evidence route count mismatch.")
        if [item.get("route_id") for item in report["route_results"]] != list(ROUTE_ORDER):
            raise ValueError("Round 2 discovery evidence route order mismatch.")
        if report.get("calibration_rows_parsed") != 0 or report.get("evaluation_rows_parsed") != 0:
            raise ValueError("Round 2 discovery evidence parsed nondevelopment rows.")
        return LockedRound2DiscoveryEvidence(report_path, checksum_path, digest, report)


class KrakenAIDrivenV2Round2DiscoveryRunner:
    ...
    def __init__(self, *, dataset_reader=None, signal_engine_factory=None, adapter_factory=None):
        ...
        self.signal_engine_factory = signal_engine_factory or KrakenAIDrivenV2Round2SignalEngine
        self.adapter_factory = adapter_factory or family_execution_adapters
        if not hasattr(self.dataset_reader, "read"):
            raise TypeError("Round 2 discovery dataset reader must provide read().")
        if not callable(self.signal_engine_factory) or not callable(self.adapter_factory):
            raise TypeError("Round 2 discovery factories must be callable.")
    ...
    def execute_development(self, frames):
        if tuple(frames) != ASSET_ORDER:
            raise ValueError("Round 2 Development frame asset order mismatch.")
        segments_by_asset = {}
        segment_rows = {}
        for asset in ASSET_ORDER:
    ...
    def _external_paths(dataset_path, evidence_root):
        ...
        if dataset == project_root or dataset.is_relative_to(project_root):
            raise ValueError("Round 2 dataset must remain outside the repository.")
        if evidence == project_root or evidence.is_relative_to(project_root):
            raise ValueError("Round 2 evidence must remain outside the repository.")
        if dataset == evidence or evidence.is_relative_to(dataset) or dataset.is_relative_to(evidence):
            raise ValueError("Round 2 dataset and evidence must not overlap.")
    ...
    def _assert_one_shot(evidence_root):
        ...
        if final.exists():
            raise FileExistsError("Round 2 discovery evidence already exists; refusing repeat.")
        if staging.exists():
            raise FileExistsError("Incomplete Round 2 discovery staging evidence exists.")
        return final, staging

    def run(self, dataset_path, evidence_root, authorization_phrase):
        if authorization_phrase != AUTHORIZATION_PHRASE:
            raise PermissionError("Exact Round 2 discovery authorization phrase is required.")
        ...
        if locked.manifest_sha256 != DATASET_MANIFEST_SHA256:
            raise ValueError("Locked Round 2 Development manifest identity mismatch.")
        if locked.calibration_rows_parsed or locked.evaluation_rows_parsed:
            raise ValueError("Locked Round 2 reader exposed nondevelopment OHLCV.")
        ...
        locked_evidence = KrakenAIDrivenV2Round2DiscoveryEvidenceLock().lock(final)
        return RecordedRound2DiscoveryEvidence(
...
def runner_declaration():
    return {
        "schema_version": SCHEMA_VERSION,
        "status": "KRAKEN_AI_V2_ROUND_2_DISCOVERY_RUNNER_IMPLEMENTED_NO_RUN_AUTHORIZATION",
        "discovery_runner_protocol_id": DISCOVERY_RUNNER_PROTOCOL_ID,
        "discovery_run_id": DISCOVERY_RUN_ID,
        "round_2_configuration_sha256": ROUND_2_CONFIGURATION_LOCK.sha256,
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Execute Kraken AI-driven v2 Round 2 Development discovery once.")
    parser.add_argument("--dataset", required=True)
    parser.add_argument("--evidence-root", required=True)
    parser.add_argument("--authorization-phrase", required=True)
    args = parser.parse_args(argv)
    result = KrakenAIDrivenV2Round2DiscoveryRunner().run(args.dataset, args.evidence_root, args.authorization_phrase)
