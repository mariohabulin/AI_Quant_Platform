"""Synthetic family execution adapters for Kraken AI-driven V2 Round 1."""
...
FAMILY_EXECUTION_COMPONENT_ID = "kraken-ai-v2-round-1-family-execution-v1"
...
PRIOR_CLOSE_LOW_10_COLUMN = "KRAKEN_AI_V2_R1_PRIOR_CLOSE_LOW_10"
EMA_50_PRIOR_COLUMN = "KRAKEN_AI_V2_R1_EMA_50_PRIOR"
...
TARGET_MODE_FROZEN_MIDLINE = "FROZEN_SIGNAL_TIME_BOLLINGER_MIDLINE"
...
ROUND_1_BASELINE_COST_PROFILE = _cost_profile("baseline")
ROUND_1_STRESS_COST_PROFILE = _cost_profile("stress")
_COST_PROFILES_BY_ID = {
    BASELINE_COST_PROFILE_ID: ROUND_1_BASELINE_COST_PROFILE,
    STRESS_COST_PROFILE_ID: ROUND_1_STRESS_COST_PROFILE,
}
...
class Round1FamilyEntryPlan:
    ...
    def __post_init__(self):
        if self.status not in (
            ...
            raise ValueError("Round 1 family entry-plan status is invalid.")
        for value, name in (
...
@dataclass(frozen=True)
class Round1FamilySyntheticPosition:
    family_id: str
    hypothesis_id: str
    execution_contract_id: str
    cost_profile_id: str
...
@dataclass(frozen=True)
class Round1FamilyExitDecision:
    status: str
    reason: str
...
@dataclass(frozen=True)
class Round1CompletedBarExitSchedule:
    status: str
    reason: str
    pending_exit_reason: str | None
    updated_position: Round1FamilySyntheticPosition
    real_order_submitted: bool = False
    performance_evaluation_executed: bool = False
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.updated_position, Round1FamilySyntheticPosition):
...
class _Round1FamilyExecutionAdapter:
    FAMILY_ID = None

    def __init__(self, *, cost_profile_id=BASELINE_COST_PROFILE_ID):
        if self.FAMILY_ID not in FAMILY_ORDER:
            raise RuntimeError("Round 1 family adapter identity is invalid.")
        if cost_profile_id not in _COST_PROFILES_BY_ID:
            raise ValueError("Round 1 family cost-profile identity is invalid.")
    ...
    def _resolved_stop(self, raw_open, setup_low, prior_atr):
        structural = setup_low - 0.25 * prior_atr
        if self.FAMILY_ID == "VOLATILITY_BREAKOUT":
            return max(structural, raw_open - 2.0 * prior_atr)
        return structural

    def _target_and_mode(self, signal_values, required_target):
        if self.FAMILY_ID in FIXED_R_FAMILIES:
            return required_target, TARGET_MODE_FIXED_R
        target = _positive_number(
            signal_values["target_anchor"], "Range signal-time target anchor"
        )
        return target, TARGET_MODE_FROZEN_MIDLINE
    ...
    def position_from_plan(self, plan):
        if not isinstance(plan, Round1FamilyEntryPlan):
            raise TypeError("Synthetic position requires a Round1FamilyEntryPlan.")
        ...
        return Round1FamilySyntheticPosition(
    ...
    def _validated_position(self, position):
        if not isinstance(position, Round1FamilySyntheticPosition):
    ...
    def _synthetic_exit(
        ...
        *,
        ...
        return Round1FamilyExitDecision(
    ...
    def complete_bar(self, position, completed_row):
        ...
        if structural_due and maximum_due:
            return Round1CompletedBarExitSchedule(
...
class KrakenAIDrivenV2Round1CapitulationExecutionAdapter(
    _Round1FamilyExecutionAdapter
):
    FAMILY_ID = "CAPITULATION_RECOVERY"


class KrakenAIDrivenV2Round1TrendPullbackExecutionAdapter(
    _Round1FamilyExecutionAdapter
):
    FAMILY_ID = "TREND_PULLBACK_CONTINUATION"


class KrakenAIDrivenV2Round1RangeMeanReversionExecutionAdapter(
    _Round1FamilyExecutionAdapter
):
    FAMILY_ID = "RANGE_MEAN_REVERSION"


class KrakenAIDrivenV2Round1VolatilityBreakoutExecutionAdapter(
    _Round1FamilyExecutionAdapter
):
